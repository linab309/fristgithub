...
void SerialApp_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  else
  {
    if ( keys & HAL_KEY_SW_1 )
    {
      ...
      uint8 sendBuf[4] = {0x31,0x32,0x33,0x34};
      
      HalUARTWrite(SERIAL_APP_PORT, "123", 3);
      HalLcdWriteString("Joystick up", HAL_LCD_LINE_2 );
      
      /*
      SerialApp_DstAddr.addrMode = Addr16Bit;
      SerialApp_DstAddr.addr.shortAddr = 0x0000; // Coordinator
      SerialApp_DstAddr.panId = 0x1122;
      SerialApp_DstAddr.endPoint = SERIALAPP_ENDPOINT;
